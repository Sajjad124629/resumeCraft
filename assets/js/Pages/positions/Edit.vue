...
const form = useForm({
    ...
    shortDescription: props.position.shortDescription || '',
    company: props.position.company || '',
    level: props.position.level || '',
    isPublic: props.position.isPublic ?? true,
    maxProjects: props.position.maxProjects ?? 3,
    projectTags: [...(props.position.projectTags || [])],
    version: props.position.version || 1,
    attributes: [...(props.position.attributes || [])],
    accessRules: [...(props.position.accessRules || [])],
});

const tagsInput = ref((props.position.projectTags || []).join(', '));
    ...
    <div>
        <div class="panel pb-3 mt-6 bg-white dark:bg-gray-800 rounded shadow p-5">
            <div class="mb-8 border-b ml-[-1.3rem] mr-[-1.3rem] border-gray-200 dark:border-gray-700">
                <div class="ml-5 mb-4">
                    <h5 class="font-semibold text-lg">{{ __('Edit Position') }}</h5>
            ...
            <form @submit.prevent="submit">
                ...
                <div class="flex justify-end items-center mt-8 mb-4">
                    ...
                    <Button type="submit" :disabled="form.processing"
                        ...
                        {{ form.processing ? __('Updating...') : __('Update Position') }}
                    </Button>
                </div>
            </form>
        </div>
