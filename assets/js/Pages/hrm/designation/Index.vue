...
const { loading, totalRows, rows, params, getData, changeServer, exportTable } = useDatatable('hrm.designation.data');
defineOptions({
    layout: AppLayout
});
...
const deleteRow = () => {
    form.delete(route('hrm.designation.destroy', form.id), {
        onSuccess: () => {
...
const importCSVForm = () => {
    form.post(route('hrm.designation.import'), {
        onSuccess: () => {
...
const deleteMultiple = () => {
    ...
    multiForm.post(route('hrm.designation.destroy.multiple'), {
        onSuccess: () => {
...
<template>
    ...
    <Head :title="__('Designation')" />
    <Modal v-model:isModalShow="isImportModalShow" :isStatic="true" :title="__('Import Designation')" width="max-w-2xl">
        <form @submit.prevent="importCSVForm">
            <div class="mb-5">
                <small><em>{{ __('The field labels marked with * are required input fields.') }}</em></small>
                <p class="mt-5">{{ __('The correct column order is (title*, description) and you must follow this.') }}
                </p>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="w-full">
                    ...
                    <a href="/sampleFile/sample_designation.csv" download>
    ...
    <Modal v-model:isModalShow="isDeleteModalShow" :isStatic="false" :title="__('Delete Designation')"
    ...
    <Modal v-model:isModalShow="isDeleteMultipleModalShow" :isStatic="false" :title="__('Delete Selected Designations')"
        modalContainerClass="text-center">
        ...
        <div class="panel pb-0 mt-6">
            <div class="mb-8 border-b mx-[-1.3rem] sm:mx-[-1.3rem] border-gray dark:border-gray-dark">
                <div class="px-5 mb-4 flex sm:items-center sm:gap-3 gap-4 w-full sm:w-auto justify-between">
                    <h4 class="font-semibold mb-2 md:mb-0  text-xl dark:text-white-light">{{ __('Designation List') }}
                    </h4>
                    <div class="flex items-center gap-3">
                        <Button type="button" class="btn btn-primary btn-sm" @click="isImportModalShow = true">
                            <HugeiconsIcon :icon="FileAddIcon" :size="20" color="currentColor" :stroke-width="1.5"
                                class="me-1" />
                            {{ __('Import Designation') }}
                        </Button>
                        <TextLink :href="route('hrm.designation.create')">
            ...
            <div class="sm:flex items-center justify-between mb-5">
                ...
                <div class="flex items-center flex-wrap justify-center">
                    <Button class="btn btn-info btn-sm me-2" @click="exportTable('csv', cols, __('Designation'))"
                        v-tippy='__("CSV")'>
                        <HugeiconsIcon :icon="Csv01Icon" :size="18" color="currentColor" :stroke-width="1.5" />
                    </Button>
                    <Button class="btn btn-success btn-sm me-2" @click="exportTable('excel', cols, __('Designation'))"
                        v-tippy='__("EXCEL")'>
                        <HugeiconsIcon :icon="DocumentAttachmentIcon" :size="18" color="currentColor"
                            :stroke-width="1.5" />
                    </Button>
                    <Button class="btn btn-warning btn-sm me-2" @click="exportTable('pdf', cols, __('Designation'))"
                        v-tippy='__("PDF")'>
                        <HugeiconsIcon :icon="Pdf02Icon" :size="18" color="currentColor" :stroke-width="1.5" />
                    </Button>

                    <Button class="btn btn-primary btn-sm me-2" @click="exportTable('print', cols, __('Designation'))"
                        v-tippy='__("PRINT")'>
                ...
                <Vue3Datatable ref="datatableRef" skin="whitespace-nowrap bh-table-hover" :rows="rows" :columns="cols"
                    ...
                    <template #action="data">
                        <div class="flex items-center gap-5">
                            <TextLink :href="route('hrm.designation.edit', data.value.id)">
