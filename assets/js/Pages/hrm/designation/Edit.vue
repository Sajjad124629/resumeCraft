...
<template>
    ...
    <Head :title="__('Edit Designation')" />
    <div>
        <div class="panel pb-3 mt-6">
            <div class="mb-8 border-b ml-[-1.3rem] mr-[-1.3rem] border-gray dark:border-gray-dark">
                <div class="ml-5 mb-4">
                    <h5 class="font-semibold text-lg">{{ __('Edit Designation') }}</h5>
                    <small><em>{{ __('The field labels marked with * are required input fields.') }}</em></small>
                </div>
            </div>
            <form @submit.prevent="form.post(route('hrm.designation.update', props.designation.id))">
                <div class="grid grid-cols-1 lg:grid-cols-1 gap-6">
                    <div>
                        <Label :isRequired="true" for="title">{{ __('Name') }}</Label>
                        <Input type="text" v-model.trim="form.title" :placeholder="__('Name')" class="form-input"
                            required />
                        <InputError :message="form.errors.title" />
                    </div>
                    <div>
                        <Label for="code">{{ __('Description') }}</Label>
                        <Textarea :rows="5" v-model.trim="form.description" :placeholder="__('Description')"
                            class="form-input" />
                ...
                <div class="flex justify-end items-center mt-8 mb-4">
                    <TextLink :href="route('hrm.designation')">
