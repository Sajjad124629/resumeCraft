...
const { loading, totalRows, rows, params, getData, changeServer, exportTable } = useDatatable('hrm.shifts.data');
...
const cols = ref([
    ...
    { field: 'grace_in', title: 'Grace In' },
    { field: 'grace_out', title: 'Grace Out' },
...
const deleteRow = () => {
    form.delete(route('hrm.shifts.destroy', form.id), {
        onSuccess: () => {
...
const deleteMultiple = () => {
    ...
    multiForm.post(route('hrm.shifts.destroy.multiple'), {
        onSuccess: () => {
...
<template>
    ...
    <Modal v-model:isModalShow="isDeleteModalShow" :isStatic="false" :title="__('Delete Shift')"
    ...
    <Modal v-model:isModalShow="isDeleteMultipleModalShow" :isStatic="false" :title="__('Delete Selected Shifts')"
        modalContainerClass="text-center">
        ...
        <div class="panel pb-0 mt-6">
            <div class="mb-8 border-b mx-[-1.3rem] sm:mx-[-1.3rem] border-gray dark:border-gray-dark">
                <div class="px-5 mb-4 flex sm:items-center sm:gap-3 gap-4 w-full sm:w-auto justify-between">
                    <h4 class="font-semibold mb-2 md:mb-0 text-xl dark:text-white-light">{{ __('Shift List') }}</h4>
                    <TextLink :href="route('hrm.shifts.create')">
            ...
            <div class="sm:flex items-center justify-between mb-5">
                ...
                <div class="flex items-center flex-wrap justify-center">
                    <Button class="btn btn-info btn-sm me-2" @click="exportTable('csv', cols, __('Shifts'))"
                        v-tippy='__("CSV")'>
                        <HugeiconsIcon :icon="Csv01Icon" :size="18" color="currentColor" :stroke-width="1.5" />
                    </Button>
                    <Button class="btn btn-success btn-sm me-2" @click="exportTable('excel', cols, __('Shifts'))"
                        v-tippy='__("EXCEL")'>
                        <HugeiconsIcon :icon="DocumentAttachmentIcon" :size="18" color="currentColor"
                            :stroke-width="1.5" />
                    </Button>
                    <Button class="btn btn-warning btn-sm me-2" @click="exportTable('pdf', cols, __('Shifts'))"
                        v-tippy='__("PDF")'>
                        <HugeiconsIcon :icon="Pdf02Icon" :size="18" color="currentColor" :stroke-width="1.5" />
                    </Button>
                    <Button class="btn btn-primary btn-sm me-2" @click="exportTable('print', cols, __('Shifts'))"
                        v-tippy='__("PRINT")'>
                ...
                <Vue3Datatable ref="datatableRef" skin="whitespace-nowrap bh-table-hover" :rows="rows" :columns="cols"
                    ...
                    <template #action="data">
                        <div class="flex items-center gap-5">
                            <TextLink :href="route('hrm.shifts.edit', data.value.id)">
